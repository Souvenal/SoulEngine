...
| **GraphicsProgram** | Shared Shader artifact produced by ShaderCompiler for graphics pipelines. It contains one linked SPIR-V code blob, vertex/fragment entry-point names, and linked pipeline-level reflection. Refers to `Shader::GraphicsProgram`. |
...
- `Core` — logging, config
- `Shader` — `Shader::Stage`, `Shader::GraphicsProgram`
