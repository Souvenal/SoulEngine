...
| **RHILoop** | Worker `std::jthread`. Waits for `RenderReady` on its slot, drains RHI task queue, executes the slot render packet, releases command observers, then sets `RHIDone`. On exit: calls `RenderDevice::WaitIdle()` before returning. |
...
| **Render packet** | `Renderer::RenderResult`, containing the `RHI::CommandList` held in `FrameSlot` until RHILoop has completed `RenderDevice::Execute()`. |
...
`Resource::Manager::TryGetReady()`. The command list stores only RHI observer
