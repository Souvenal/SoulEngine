...
class RayTracingRenderer final : public IRenderer {
    ...
    [[nodiscard]] auto Render(const Scene::SceneSnapshot& Scene) -> std::expected<RenderResult, ErrorMessage> override {
        ...
        for (const auto& Renderable : Scene.Renderables) {
            ...
            Instances.push_back(RHI::AccelerationStructureInstance{
                ...
                .Transform = ToRowMajorTransform(Renderable.WorldTransform),
            });
        }
        if (Instances.empty())
            return Result;
  ...
  private:
    ...
    [[nodiscard]] static auto ToRowMajorTransform(const hlslpp::float4x4& Matrix) -> RHI::RowMajorTransform3x4 {
        return {
            .M00 = Matrix[0].x, .M01 = Matrix[0].y, .M02 = Matrix[0].z, .M03 = Matrix[0].w,
            .M10 = Matrix[1].x, .M11 = Matrix[1].y, .M12 = Matrix[1].z, .M13 = Matrix[1].w,
            .M20 = Matrix[2].x, .M21 = Matrix[2].y, .M22 = Matrix[2].z, .M23 = Matrix[2].w,
        };
    }
