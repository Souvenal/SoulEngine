...
| **TestRenderer** | Prototype concrete renderer. Owns async pipeline/texture handles and emits variant commands when dependencies are ready. |
...
- `Resource` — async texture and pipeline requests
...
- Pipeline and texture lifetime flow through `Resource::Manager` handles; sync texture/pipeline creation is not done in renderer attach or render-frame code.
