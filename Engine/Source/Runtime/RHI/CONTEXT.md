...
| **Transient buffers and arenas** | RenderDevice allocates typed logical transient uniform or shader-storage buffer handles. Renderers write byte snapshots for those handles into command lists; the RHI thread resolves each handle to a range in the backend-owned host-visible, host-coherent arena for the current frame slot. |
...
| **Draw shader binding** | Draw-scope association from a shader binding name to a typed RHI resource observer or transient constant-buffer handle. Backends resolve it through the draw command's expected graphics pipeline reflection. |
...
- Sampled textures and samplers are assigned through **Shader parameters**. Constant-buffer bindings receive a command-list-scoped transient constant-buffer handle. Resource arrays are assigned through `SetResourceArray`; runtime sampled texture arrays currently use `ResourceArray<SampledTexture>`, while individual slot selection belongs to material/object data.
...
- `AllocateTransientConstantBuffer` and `AllocateTransientShaderStorageBuffer` allocate typed logical RHI handles. They do not create backend-native memory or expose offsets; `RHIPass` and `RHINonRenderingPass` record data snapshots through matching write commands, and backend command execution resolves the handles to physical arena ranges.
...
| `RHIPass::WriteTransientConstantBuffer` | Called by render code before `Execute()`; backend consumes the copied draw-scope write on `RHILoop` through dynamic uniform-buffer descriptor binding. |
