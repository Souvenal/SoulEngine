...
| **MeshComponent** | Optional component that associates a Scene Entity with mesh content to be rendered. Its Authoring State is an asset path relative to the current application Assets directory plus an optional scene-local `MaterialInstance` ID. Renderer-specific mesh resource refs, uploads, and GPU representations are renderer-owned rather than component state. |
